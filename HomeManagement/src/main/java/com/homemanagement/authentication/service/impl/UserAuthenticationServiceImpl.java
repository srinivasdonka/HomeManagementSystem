...
class CheckRoles {
	...
	public static boolean checkPrivilliges(UserDetailsDecorator currentUser, boolean checkUser) {
		...
		try (MongoClient mongoClient = new MongoClient(new MongoClientURI(HomeManagementConstant.MONGO_URL.getKey()))) {
			...
			List<Roles> roleList = mongoTemplate.findAll(Roles.class);
			for (Roles roles : roleList) {
				if (roles.getName().equals(currentUser.getRole())) {
					LOGGER.info(" Current role is  :" + currentUser.getRole());
					final Query query = Query
							.query(Criteria.where(HomeManagementKeyConstant.USERID).is(currentUser.getUser().getId()));
					...
					for (PrivilegesMapping privilliges : privilegesList) {
						LOGGER.info(" Current permission is  :" + privilliges.getName());
						if (privilliges.getUser_id().equals(currentUser.getUser().getId())) {
							checkUser = true;
						} else {
							LOGGER.debug(" Permissions doesnot have perticular Role");
						}
					}
				}
			}
...
public class UserAuthenticationServiceImpl implements UserAuthenticationService {
	...
	private boolean userIsAtLeastAdmin(UserDetailsDecorator currentUser) {
		...
		for(Roles role:roles) {
			if(currentUser.getUser().getRole_id().equals(role.getId())) {
				if(role.getName().equals(HomeManagementConstant.ADMIN) && role.getName().equals(HomeManagementConstant.ADMIN)) {
					return true;
				}
				else {
					return false;
	...
	public boolean canAccessUserWithUserDto(Object obj, UserDTO userDTO, @Nullable boolean adminRequiered) {
		...
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			...
			UserDetailsDecorator currentUser = (UserDetailsDecorator)obj;
			if(userIsXadaAdmin(currentUser) || userDTO.getUsername().equals(currentUser.getUsername())) {
				return true;
			}
	...
	public boolean canAccessUserWithUser(Object obj, User user, @Nullable boolean adminRequiered) {
		...
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			...
			UserDetailsDecorator currentUser = (UserDetailsDecorator)obj;
			if(userIsXadaAdmin(currentUser) || user.getUsername().equals(currentUser.getUsername())) {
				return true;
			}
	...
	public boolean canAccessUserWithCompanyId(Object obj, String companyId, boolean adminRequiered) {
		boolean checkUser = false;
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			return checkUser;
		}else {
	...
	public boolean canAccessUserWithCompanyMaster(Object obj, CompanyMaster companyMaster, boolean adminRequiered) {
		boolean checkUser = false;
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			return checkUser;
		}else {
	...
	public boolean canAccessUserWithEmail(Object obj, String email, boolean adminRequiered) {
		boolean checkUser = false;
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			return checkUser;
		}else {
	...
	public boolean canAccessUserWithListUserDTO(Object obj, List<UserDTO> userDTOs, boolean adminRequiered) {
		boolean checkUser = false;
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			return checkUser;
		}else {
	...
	public boolean canAccessUserWithEmailVO(Object obj, EmailVo emailVO, boolean adminRequiered) {
		boolean checkUser = false;
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			return checkUser;
		}else {
	...
	public boolean canAccessUserWithUserId(Object obj, String id, boolean adminRequiered) {
		boolean checkUser = false;
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			return checkUser;
		}else {
	...
	public boolean canAccessUserWithUserDTOSameUser(Object obj, UserDTO userDTO, boolean adminRequiered) {
		boolean checkUser = false;
		if(obj.equals(HomeManagementConstant.ANONYMOUS_USER)) {
			return checkUser;
		}else {
